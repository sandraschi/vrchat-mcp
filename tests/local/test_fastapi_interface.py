...
class FastAPITester:
    ...
    def __init__(self, base_url: str = "http://127.0.0.1:8000", server_command: list | None = None):
    ...
    def make_request(self, method: str, endpoint: str, **kwargs) -> requests.Response:
        ...
        # Set default timeout for S113 compliance
    ...
    async def test_api_endpoints(self) -> bool:
        ...
        test_endpoints = [
            ...
            },
            {
                ...
                "method": "POST",
                "data": {"avatar_id": "test_avatar_123", "parameter": "TestParameter", "value": 0.5},
    ...
    async def test_error_handling(self) -> bool:
        ...
        for test in error_tests:
            try:
                response = self.make_request(
                    test["method"], test["path"], json=test["data"], headers={"Content-Type": "application/json"}
                )

                if response.status_code != test["expected_status"]:
                    logger.error(
                        f"{test['description']} returned {response.status_code}, expected {test['expected_status']}"
                    )
                    continue
