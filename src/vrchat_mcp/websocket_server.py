...
class WebSocketServer:
    ...
    async def _handle_auth(self, data: dict, websocket: WebSocketServerProtocol) -> dict:
        ...
        if not self.auth_token:
            return {"type": "auth_response", "authenticated": True, "message": "Authentication not required"}

        token = data.get("token")
        if token == self.auth_token:
            self.client_info[websocket]["authenticated"] = True
            return {"type": "auth_response", "authenticated": True, "message": "Authentication successful"}
        else:
            return {
                "type": "auth_response",
                "authenticated": False,
    ...
    async def _handle_load_avatar(self, data: dict, websocket: WebSocketServerProtocol) -> dict:
        ...
        if not self.avatar_manager:
            return {"type": "error", "code": "not_available", "message": "Avatar management is not available"}

        try:
            request = AvatarLoadRequest(avatar_id=data["avatar_id"], parameters=data.get("parameters", {}))

            success = await self.avatar_manager.load_avatar(request)

            return {"type": "load_avatar_response", "success": success, "avatar_id": request.avatar_id}
            ...
            return {"type": "error", "code": "load_failed", "message": f"Failed to load avatar: {e!s}"}
    ...
    async def _handle_set_parameter(self, data: dict, websocket: WebSocketServerProtocol) -> dict:
        ...
        try:
            ...
            if not current_avatar:
                return {"type": "error", "code": "no_avatar_loaded", "message": "No avatar is currently loaded"}

            await self.avatar_manager.set_parameter(
                current_avatar, request.name, request.value, immediate=request.immediate
            )
            ...
            return {"type": "error", "code": "set_parameter_failed", "message": f"Failed to set parameter: {e!s}"}
    ...
    async def _handle_set_parameters(self, data: dict, websocket: WebSocketServerProtocol) -> dict:
        ...
        try:
            ...
            await self.avatar_manager.set_parameters(current_avatar, parameters, immediate=immediate)

            return {"type": "set_parameters_response", "success": True, "parameters_updated": len(parameters)}
        except KeyError as e:
            return {
                "type": "error",
                "code": "missing_field",
            ...
            return {"type": "error", "code": "set_parameters_failed", "message": f"Failed to set parameters: {e!s}"}
    ...
    async def _handle_play_animation(self, data: dict, websocket: WebSocketServerProtocol) -> dict:
        ...
        try:
            ...
            return {"type": "play_animation_response", "success": True, "animation": request.name}
        except KeyError as e:
            return {
                "type": "error",
                "code": "missing_field",
            ...
            return {"type": "error", "code": "play_animation_failed", "message": f"Failed to play animation: {e!s}"}
    ...
    async def _handle_set_expression(self, data: dict, websocket: WebSocketServerProtocol) -> dict:
        ...
        try:
            ...
            return {"type": "error", "code": "set_expression_failed", "message": f"Failed to set expression: {e!s}"}
    ...
    async def _handle_load_preset(self, data: dict, websocket: WebSocketServerProtocol) -> dict:
        ...
        try:
            ...
            if not success:
                return {"type": "error", "code": "preset_not_found", "message": f"Preset not found: {preset_name}"}

            return {"type": "load_preset_response", "success": True, "preset": preset_name}
        except KeyError as e:
            return {
                "type": "error",
                "code": "missing_field",
            ...
            return {"type": "error", "code": "load_preset_failed", "message": f"Failed to load preset: {e!s}"}
    ...
    async def _handle_save_preset(self, data: dict, websocket: WebSocketServerProtocol) -> dict:
        ...
        try:
            ...
            if not preset:
                return {"type": "error", "code": "save_preset_failed", "message": "Failed to save preset"}
            ...
            return {"type": "error", "code": "save_preset_failed", "message": f"Failed to save preset: {e!s}"}
    ...
    async def _handle_delete_preset(self, data: dict, websocket: WebSocketServerProtocol) -> dict:
        ...
        try:
            ...
            return {"type": "delete_preset_response", "success": True, "preset": preset_name}
        except KeyError as e:
            return {
                "type": "error",
                "code": "missing_field",
            ...
            return {"type": "error", "code": "delete_preset_failed", "message": f"Failed to delete preset: {e!s}"}
    ...
    async def _handle_list_presets(self, data: dict, websocket: WebSocketServerProtocol) -> dict:
        ...
        try:
            ...
            return {"type": "list_presets_response", "presets": [p.to_dict() for p in presets]}
        except Exception as e:
            logger.error(f"Error listing presets: {e}", exc_info=True)
            return {"type": "error", "code": "list_presets_failed", "message": f"Failed to list presets: {e!s}"}
    ...
    async def _handle_get_avatar_state(self, data: dict, websocket: WebSocketServerProtocol) -> dict:
        ...
        try:
            ...
            if not state:
                return {"type": "error", "code": "avatar_not_found", "message": f"Avatar not found: {avatar_id}"}

            return {"type": "avatar_state", "avatar_id": avatar_id, "state": state.to_dict()}
        except Exception as e:
            logger.error(f"Error getting avatar state: {e}", exc_info=True)
            return {"type": "error", "code": "get_avatar_state_failed", "message": f"Failed to get avatar state: {e!s}"}
    ...
    async def _handle_get_connection_stats(self, data: dict, websocket: WebSocketServerProtocol) -> dict:
        ...
        if not self.osc_manager:
            return {"type": "error", "code": "not_available", "message": "OSC management is not available"}

        try:
            stats = self.osc_manager.get_connection_stats()
            return {"type": "connection_stats", "stats": stats.to_dict()}
        except Exception as e:
            logger.error(f"Error getting connection stats: {e}", exc_info=True)
            return {
                "type": "error",
                "code": "get_connection_stats_failed",
                "message": f"Failed to get connection stats: {e!s}",
            }
    ...
    async def _send_error(
        self, websocket: WebSocketServerProtocol, code: str, message: str, details: dict | None = None
    ) -> None:
