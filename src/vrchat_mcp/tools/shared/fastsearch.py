...
class FastSearch:
    ...
    async def index_parameter(
        self, param_name: str, param_type: str = "Float", source: str = "unknown", first_seen: float | None = None
    ) -> None:
    ...
    async def index_osc_endpoint(self, endpoint: str, description: str = "", first_seen: float | None = None) -> None:
    ...
    def search_parameters(self, query: str) -> list[dict[str, Any]]:
        ...
        for param_name, param_data in self.parameters.items():
            if query_lower in param_name.lower():
                results.append({"name": param_name, **param_data})
    ...
    def search_osc_endpoints(self, query: str) -> list[dict[str, Any]]:
        ...
        for endpoint, endpoint_data in self.osc_endpoints.items():
            if query_lower in endpoint.lower() or query_lower in endpoint_data.get("description", "").lower():
                results.append({"endpoint": endpoint, **endpoint_data})
    ...
    def get_endpoint_info(self, endpoint: str) -> dict[str, Any] | None:
