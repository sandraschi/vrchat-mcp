...
from ..models import AvatarState, ParameterValue, MessageDirection, MessageRecord
...
class AvatarPlugin(Plugin):
    ...
    @tool(
        ...
        examples=[
            ...
            {"description": "Interpolate a parameter", "code": "set_avatar_parameter('avatar1', 'Viseme', 1.0, interpolate=True, duration=1.0, easing='ease_in_out')"}
        ]
    )
        ...
        try:
            ...
            if interpolate and hasattr(self.mcp, 'interpolation'):
                ...
                if hasattr(self.mcp, 'osc_inspector'):
                    await self.mcp.osc_inspector.send_message(
    ...
    @tool(
        ...
        examples=[
            {"description": "Get a parameter value", "code": "get_avatar_parameter('avatar1', 'Viseme')"}
        ]
    )
    async def get_avatar_parameter(
        self,
        avatar_id: str,
        parameter: str
    ) -> Dict[str, Any]:
        ...
        return {
            "value": self.avatars[avatar_id].parameters[parameter],
            "exists": True
        }
    ...
    @tool(
        ...
        examples=[
            ...
            {"description": "Apply with interpolation", "code": "apply_avatar_preset('avatar1', 'tpose', interpolate=True, duration=1.0)"}
        ]
    )
    async def apply_avatar_preset(
        ...
        for param_name, value in preset.parameters.items():
            await self.set_avatar_parameter(
                avatar_id,
                param_name,
                value,
    ...
    @tool(
        ...
        args={
            ...
            "include_expressions": {"type": "boolean", "description": "Whether to include expressions", "default": False}
        },
        returns={"success": "boolean", "message": "string"},
        examples=[
            {"description": "Save current state as a preset", "code": "save_avatar_preset('my_preset')"}
        ]
    )
    ...
    @tool(
        ...
        examples=[
            {"description": "Get parameter history", "code": "get_parameter_history('Viseme', limit=50)"}
        ]
    )
    async def get_parameter_history(
        ...
        return {
            "parameter": parameter,
            "history": self.parameter_history[parameter][-limit:]
        }
