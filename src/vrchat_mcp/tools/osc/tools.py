...
from ...models import OSCBundle, OSCMessage
...
class OSCManager:
                ...
                if loop.is_running():
                    task = asyncio.create_task(
                        fast_search.index_parameter(
                            param_name=param_name, param_type=param_type, source="discovered", first_seen=loop.time()
                        )
                    )
    ...
    def _index_osc_endpoint(self, endpoint: str, description: str = "") -> None:
        ...
        try:
            ...
            if loop.is_running():
                task = asyncio.create_task(
                    fast_search.index_osc_endpoint(endpoint=endpoint, description=description, first_seen=loop.time())
                )
                self._background_tasks.add(task)
                task.add_done_callback(self._background_tasks.discard)
        except RuntimeError:
    ...
    async def connect(self) -> None:
        ...
        if self.client is None:
            self.client = udp_client.SimpleUDPClient(self.config["send_host"], self.config["send_port"])
            logger.info(
                "OSC client connected", extra={"host": self.config["send_host"], "port": self.config["send_port"]}
            )
        ...
        if self.server is None:
            ...
            server = osc_server.AsyncIOOSCUDPServer(
                (self.config["receive_host"], self.config["receive_port"]), self.dispatcher, self.loop
            )
            self.server = server
            _transport, _ = await server.create_serve_endpoint()
            logger.info(
                "OSC server started", extra={"host": self.config["receive_host"], "port": self.config["receive_port"]}
            )
    ...
    async def send_parameter(
        self, parameter_name: str, value: bool | float | int | str, avatar_id: str | None = None
    ) -> None:
        ...
        logger.debug(
            "Sent parameter update", extra={"avatar_id": avatar_id, "parameter": parameter_name, "value": value}
        )
    ...
    def _handle_parameter_change(self, address: str, *args) -> None:
        ...
        if self.current_avatar_id and self.current_avatar_id in self.avatars:
            ...
            logger.debug(
                "Parameter changed",
                extra={"avatar_id": self.current_avatar_id, "parameter": parameter_name, "value": value},
            )
    ...
    def _handle_avatar_change(self, address: str, avatar_id: str) -> None:
        ...
        if avatar_id not in self.avatars:
            self.avatars[avatar_id] = {"parameters": {}, "last_seen": asyncio.get_event_loop().time()}
    ...
    # Utility methods
    def get_avatar_parameters(self, avatar_id: str | None = None) -> dict[str, Any]:
    ...
    def get_parameter_value(self, parameter_name: str, avatar_id: str | None = None, default: Any = None) -> Any:
