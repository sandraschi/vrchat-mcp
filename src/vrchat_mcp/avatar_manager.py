...
from .interpolation import InterpolationManager, EasingFunction
...
class AvatarManager:
    ...
    def __init__(self, osc_manager: OSCManager):
        ...
        self._presets_file: Optional[str] = None
        ...
        asyncio.create_task(self._interpolation_manager.start())
    
    async def load_avatar(self, request: Union[str, AvatarLoadRequest]) -> bool:
        ...
        async with self._state_lock:
            try:
                ...
                if request.parameters:
                    asyncio.create_task(self._set_parameters_after_delay(
                        request.avatar_id, 
                        request.parameters,
    ...
    async def _set_parameters_after_delay(
        self, 
        avatar_id: str, 
    ...
    async def play_animation(
        self,
        avatar_id: str,
        animation_request: Union[str, AnimationRequest]
    ) -> None:
        ...
        async with self._state_lock:
            ...
            state.active_animations = [
                anim for anim in state.active_animations 
                if anim.get('name') != animation_request.name
            ]
    ...
    async def set_expression(
        self,
        avatar_id: str,
        expression_request: Union[str, ExpressionRequest]
    ) -> None:
        ...
        async with self._state_lock:
            ...
            state.active_expressions = [
                expr for expr in state.active_expressions
                if expr.get('name') != expression_request.name
            ]
    ...
    def load_presets_from_file(self, filepath: str) -> None:
        ...
        try:
            with open(filepath, 'r') as f:
                data = json.load(f)
                self.presets = {
                    name: AvatarPreset.from_dict(preset_data)
    ...
    def _save_presets_async(self) -> None:
        ...
        asyncio.create_task(self._save_presets_to_file(self._presets_file, presets_data))
    ...
    async def get_avatar_state(self, avatar_id: str) -> Optional[AvatarState]:
        """Get the current state of an avatar."""
        async with self._state_lock:
            return self.avatar_states.get(avatar_id)
    
    async def save_avatar_state(self, name: str) -> Optional[AvatarPreset]:
        ...
        async with self._state_lock:
            if avatar_id in self.avatar_states:
                ...
                if reset_parameters:
                    ...
                    self.avatar_states[avatar_id] = AvatarState(
                        avatar_id=avatar_id,
    ...
    async def get_parameter(self, avatar_id: str, param_name: str) -> Optional[Any]:
    ...
    def search_presets(self, query: str) -> List[AvatarPreset]:
        ...
        for preset in self.presets.values():
            if (query in preset.name.lower() or
                ...
                query in preset.description.lower()):
