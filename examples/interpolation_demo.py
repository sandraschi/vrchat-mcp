...
class AvatarManager:
    ...
    async def set_parameter(
        ...
        if interpolate:
            logger.info(f"Interpolating {name} to {value:.2f} over {duration:.1f}s with {easing} easing")
        else:
    ...
    async def set_parameters(
        ...
        for name, value in parameters.items():
            await self.set_parameter(avatar_id, name, value, interpolate, duration, easing)
...
async def demo_single_parameter(mcp: MCPClient) -> None:
    ...
    await mcp.avatar.set_parameter(avatar_id, param_name, 1.0, interpolate=True, duration=2.0)
    ...
    await mcp.avatar.set_parameter(
        avatar_id, param_name, 0.0, interpolate=True, duration=1.5, easing="ease_in_out_sine"
    )
...
async def demo_multiple_parameters(mcp: MCPClient) -> None:
    ...
    await mcp.avatar.set_parameters(
        avatar_id,
        parameters={"Viseme": 1.0, "GestureLeft": 2.0, "GestureRight": 3.0},
        interpolate=True,
        duration=2.0,
    ...
    await mcp.avatar.set_parameters(
        avatar_id,
        parameters={"Viseme": 0.0, "GestureLeft": 0.0, "GestureRight": 0.0},
        interpolate=True,
        duration=1.5,
        easing="bounce_out",
    )
...
async def demo_easing_functions(mcp: MCPClient) -> None:
    ...
    for easing in easing_functions:
        # Move up
        await mcp.avatar.set_parameter(avatar_id, param_name, 1.0, interpolate=True, duration=1.5, easing=easing)
        await asyncio.sleep(1.7)  # Slight extra delay

        # Move down
        await mcp.avatar.set_parameter(avatar_id, param_name, 0.0, interpolate=True, duration=1.5, easing=easing)
        await asyncio.sleep(1.7)  # Slight extra delay
