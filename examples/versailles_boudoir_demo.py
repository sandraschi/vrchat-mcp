...
from vrchat_mcp.models import NPCAction, NPCState, OSCMessage
from vrchat_mcp.tools import NPCManager, OSCManager, fast_search
...
class VersaillesBoudoirDemo:
    ...
    def __init__(self):
        ...
        self.osc = OSCManager({"send_port": 9000, "receive_port": 9001, "auto_index_parameters": True})

        self.npc_manager = NPCManager()
        self.scene_state = {"time_of_day": "afternoon", "mood": "elegant", "active_npcs": {}}
        ...
        for npc_id, npc_data in self.npcs.items():
            task = asyncio.create_task(
                fast_search.index_npc(
                    ...
                    tags=[f"mood:{mood}" for mood in npc_data["moods"]] + npc_data["interests"],
                )
            )
        ...
        for param, desc, tags in scene_params:
            task = asyncio.create_task(
                fast_search.index_parameter(
                    param_name=f"boudoir/{param}", param_type="float", description=desc, tags=tags
                )
            )
            self._background_tasks.add(task)
            task.add_done_callback(self._background_tasks.discard)
    ...
    async def set_scene_mood(self, mood: str) -> None:
        ...
        if mood == "romantic":
            ...
            await self.osc.send_message(
                OSCMessage("/avatar/parameters/Boudoir/Lighting/Color", [1.0, 0.8, 0.9])
            )  # Soft pink
        elif mood == "mysterious":
            await self.osc.send_message(OSCMessage("/avatar/parameters/Boudoir/Lighting/Intensity", 0.5))
            await self.osc.send_message(
                OSCMessage("/avatar/parameters/Boudoir/Lighting/Color", [0.5, 0.3, 0.8])
            )  # Deep purple
        else:  # elegant
            await self.osc.send_message(OSCMessage("/avatar/parameters/Boudoir/Lighting/Intensity", 1.0))
            await self.osc.send_message(
                OSCMessage("/avatar/parameters/Boudoir/Lighting/Color", [1.0, 0.95, 0.9])
            )  # Warm white
    ...
    async def start_npc_interaction(self, npc_id: str, interaction_type: str) -> None:
        ...
        if interaction_type == "greeting":
            await self.npc_manager.execute_action(
                npc_id=npc_data["id"], action=NPCAction(type="animation", name="wave_hand", duration=2.0)
            )
            # Play greeting sound
            await self.osc.send_message(
                OSCMessage("/avatar/parameters/Boudoir/Audio/PlayOneShot", f"greeting_{random.randint(1, 3)}")  # noqa: S311
            )
    ...
    async def run_demo_sequence(self) -> None:
        ...
        for npc_id in self.npcs:
            await self.npc_manager.teleport_npc(
                ...
                position=(random.uniform(-2, 2), 0, random.uniform(-2, 2)),  # noqa: S311
            )
