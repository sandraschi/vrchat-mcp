...
class DebugUI:
    ...
    def __init__(
        self, osc_inspector: OSCInspector, host: str = "127.0.0.1", port: int = 8765, web_root: str | Path | None = None
    ):
    ...
    async def handle_ws_message(self, ws: web.WebSocketResponse, message: dict[str, Any]) -> None:
        ...
        if msg_type == "get_messages":
            ...
            await self.send(ws, {"type": "messages", "messages": messages})
            ...
            await self.send(ws, {"type": "filtered_messages", "messages": filtered})

        elif msg_type == "send_message":
            # Send an OSC message
            try:
                self.osc_inspector.send_message(message["address"], *message.get("args", []))
                await self.send(ws, {"type": "message_sent"})
            except Exception as e:
                await self.send(ws, {"type": "error", "message": f"Failed to send message: {e}"})
    ...
    def on_osc_message(self, record: MessageRecord) -> None:
        ...
        task = asyncio.create_task(self.broadcast({"type": "new_message", "message": record.to_dict()}))
        self._background_tasks.add(task)
        task.add_done_callback(self._background_tasks.discard)

    def on_status_update(self) -> None:
        """Handle status updates."""
        status_data = self.osc_inspector.get_statistics()
        task = asyncio.create_task(self.broadcast({"type": "status_update", "status": status_data}))
        self._background_tasks.add(task)
        task.add_done_callback(self._background_tasks.discard)
