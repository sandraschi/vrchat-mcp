...
class NPCManager:
    ...
    def _init_language_model(self) -> None:
        ...
        self.language_model = {"model": self.config["default_model"], "temperature": 0.7, "max_tokens": 150}

        logger.info("Initialized language model", extra={"model": self.config["default_model"]})

    async def start_conversation(self, npc_id: str, user_id: str, initial_message: str | None = None) -> dict[str, Any]:
        ...
        if conversation_id not in self.conversations:
            ...
            self.conversations[conversation_id].append({"role": "system", "content": system_prompt})
            ...
            logger.info("Started new conversation", extra={"npc_id": npc_id, "user_id": user_id})
        ...
        return {"status": "error", "error": "Conversation already exists", "conversation_id": conversation_id}

    async def add_message(self, conversation_id: str, role: str, content: str) -> dict[str, Any]:
        """Add a message to an existing conversation."""
        if conversation_id not in self.conversations:
            return {"status": "error", "error": "Conversation not found", "conversation_id": conversation_id}

        # Add the message to the conversation history
        self.conversations[conversation_id].append(
            {"role": role, "content": content, "timestamp": asyncio.get_event_loop().time()}
        )

        # Trim history if needed
        if (
            len(self.conversations[conversation_id]) > self.config["max_history"] * 2 + 1
        ):  # *2 for user/assistant pairs, +1 for system
            self.conversations[conversation_id] = [
        ...
        logger.debug(
            ...
            extra={"conversation_id": conversation_id, "role": role, "content_length": len(content)},
        )

        return {"status": "success"}

    async def get_response(self, conversation_id: str, user_message: str | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            return {"status": "success", "response": response, "conversation_id": conversation_id}
    ...
    async def _generate_response(self, messages: list[dict[str, str]]) -> str:
        ...
        last_user_message = next(
            (msg["content"] for msg in reversed(messages) if msg["role"] == "user"), "Hello! How can I help you today?"
        )
    ...
    async def end_conversation(self, conversation_id: str) -> dict[str, Any]:
        ...
        if conversation_id in self.conversations:
            ...
            return {"status": "success", "message": "Conversation ended", "conversation_id": conversation_id}
    ...
    def update_npc_state(self, npc_id: str, updates: dict[str, Any]) -> dict[str, Any]:
