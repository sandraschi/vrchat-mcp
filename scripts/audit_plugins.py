...
PLUGINS_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 
                          "src", "vrchat_mcp", "plugins")
...
class PluginAudit:
    ...
    def load_module(self) -> bool:
        ...
        try:
            spec = importlib.util.spec_from_file_location(
                f"vrchat_mcp.plugins.{self.plugin_name}", 
                self.filepath
            )
            ...
            self.issues.append(("error", f"Failed to import module: {str(e)}"))
            return False
    ...
    def parse_ast(self) -> None:
        ...
        try:
            ...
            self.issues.append(("error", f"Failed to parse AST: {str(e)}"))
    ...
    def check_plugin_class(self) -> None:
        ...
        for name, obj in inspect.getmembers(self.module, inspect.isclass):
            if (name.endswith('Plugin') and 
                    ...
                    obj.__module__ == self.module.__name__):
                plugin_class = obj
                break
        ...
        for name, method in inspect.getmembers(plugin_class, inspect.isfunction):
            ...
            if hasattr(method, '_event_listeners'):
                self.event_listeners.extend([
                    {"name": name, "event_type": l.get('event_type', 'unknown')}
                    for l in method._event_listeners
                ])
    ...
    def audit_tool(self, name: str, method: Any) -> None:
        ...
        if missing_metadata:
            self.issues.append(("warning", 
                              f"Tool '{name}' is missing required metadata: {', '.join(missing_metadata)}"))
