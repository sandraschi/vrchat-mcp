...
class AvatarManager:
            ...
            if self.osc_manager:
                ...
                if avatar_id not in self.avatars:
                    self.avatars[avatar_id] = AvatarState(
                        avatar_id=avatar_id, parameters={}, loaded_at=asyncio.get_event_loop().time()
                    )

                logger.info(f"Avatar load requested: {avatar_id}")
                return {"status": "success", "avatar_id": avatar_id, "message": f"Avatar '{avatar_id}' load requested"}
            else:
                return {"status": "error", "error": "OSC manager not available", "avatar_id": avatar_id}

        except Exception as e:
            logger.error(f"Failed to load avatar {avatar_id}: {e}")
            return {"status": "error", "error": str(e), "avatar_id": avatar_id}

    async def get_avatar_state(self, avatar_id: str | None = None) -> dict[str, Any]:
        """
        ...
        if not avatar_id:
            return {"status": "error", "error": "No avatar ID specified and no current avatar"}

        if avatar_id not in self.avatars:
            return {"status": "error", "error": f"Avatar '{avatar_id}' not found", "avatar_id": avatar_id}
            ...
            if interpolate and self.interpolation:
                ...
                task = asyncio.create_task(
                    self._interpolate_parameter(
                        avatar_id, parameter_name, float(current_value), float(value), duration, easing
                    )
                )
    ...
    async def get_parameter(
        self, avatar_id: str, parameter_name: str, default: Any = None
    ...
    async def _interpolate_parameter(
        self, avatar_id: str, parameter_name: str, start_value: float, end_value: float, duration: float, easing: str
    ) -> None:
        """
    ...
    def get_current_avatar(self) -> str | None:
