  ...
  const ComponentStatus = ({
    ...
    <div className="flex items-center justify-between p-2 rounded bg-slate-900/30 border border-slate-800">
      ...
      <div className="flex items-center">
        <span
          className={`text-xs mr-2 ${active ? "text-emerald-400" : "text-rose-400"}`}
        >
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400">
            OSC connectivity and system health for VRChat-bridge.
          </p>
        </div>
        <div
          className={`px-4 py-1 rounded-full text-xs font-semibold ${status?.status === "running" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-rose-500/10 text-rose-400 border border-rose-500/20"}`}
        >
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <Card className="border-slate-800 bg-slate-950/50">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 text-white">
            ...
            <Network
              className={`h-4 w-4 ${status?.components?.osc ? "text-emerald-500" : "text-slate-500"}`}
            />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-white">
              {status?.components?.osc ? "Active" : "Missing"}
            </div>
        ...
        <Card className="border-slate-800 bg-slate-950/50">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 text-white">
            ...
            <Cpu
              className={`h-4 w-4 ${status?.components?.avatar ? "text-blue-500" : "text-slate-500"}`}
            />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-white">
              {status?.components?.avatar ? "Ready" : "Error"}
            </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card className="border-slate-800 bg-slate-950/50">
          ...
          <CardContent className="space-y-2">
            <ComponentStatus
              name="OSC Manager"
              active={!!status?.components?.osc}
            />
            <ComponentStatus
              name="Avatar Manager"
              active={!!status?.components?.avatar}
            />
            <ComponentStatus
              name="OSC Inspector"
              active={!!status?.components?.inspector}
            />
            <ComponentStatus
