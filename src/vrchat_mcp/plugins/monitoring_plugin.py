...
class ConnectionStatus:
    ...
    async def handle_state(self, state: ConnectionState, error: Optional[str] = None) -> bool:
        ...
        logger.info(f"Connection '{self.name}' state changed: {old_state.name} -> {state.name}" + 
...
class MonitoringPlugin(Plugin):
    ...
    async def update_connection_state(
        ...
        if (self.auto_reconnect and 
            state == ConnectionState.DISCONNECTED and 
            status.reconnect_attempts < self.max_reconnect_attempts):
            
            asyncio.create_task(self._handle_reconnection(name))
    ...
    async def _handle_reconnection(self, name: str) -> None:
        ...
        logger.info(f"Attempting to reconnect {name} (attempt {status.reconnect_attempts}/{self.max_reconnect_attempts}) in {delay:.1f}s...")
        ...
        try:
            ...
            await self.update_connection_state(
                ...
                f"Reconnection attempt {status.reconnect_attempts}"
            )
            ...
            if success:
                ...
                await self.update_connection_state(
                    ...
                    f"Reconnection attempt {status.reconnect_attempts} failed"
                )
    ...
    @tool(
        ...
        category="Monitoring",
        args={
            "name": {"type": "string", "description": "Name of the connection (omit for all)", "default": None}
        },
    ...
    async def get_connection_status(self, name: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            "connections": [
                self._connection_to_dict(conn) for conn in self.connections.values()
            ]
        }
    ...
    def _connection_to_dict(self, conn: ConnectionStatus) -> Dict[str, Any]:
        ...
        return {
            ...
            "next_reconnect_attempt": (
                conn.last_disconnected + 
                min(self.reconnect_delay * (2 ** conn.reconnect_attempts), 300)
    ...
    @tool(
        ...
        category="Monitoring",
        args={
            "enabled": {"type": "boolean", "description": "Whether to enable auto-reconnect"}
        },
